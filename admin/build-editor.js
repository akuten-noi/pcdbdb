(function(){
  ...
  function Control(props){
    const builds=plain(props.value); const emit=next=>props.onChange(next);
    ...
    return h('div',{},[
      builds.map((raw,i)=>{const b=Object.assign({},EMPTY,raw);const p=DATA.find(x=>x.name===b.pokemon);const total=POINTS.reduce((s,[k])=>s+(Number(b[k])||0),0);const items=allowedItems(p);return h('section',{key:i,style:{border:'1px solid #d0d5dd',borderRadius:'10px',padding:'16px',marginBottom:'18px',background:'#fff'}},[
        h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'14px'}},[h('strong',{},`育成論 ${i+1}${b.pokemon?'｜'+b.pokemon:''}`),h('button',{type:'button',onClick:()=>remove(i),style:{border:0,background:'#b42318',color:'#fff',padding:'7px 12px',borderRadius:'6px'}},'削除')]),
        field('公開する',h('input',{type:'checkbox',checked:b.published!==false,onChange:e=>update(i,'published',e.target.checked)})),
        field('ポケモン名',select(b.pokemon,v=>{const next=normalized();next[i].pokemon=v;next[i].ability='';next[i].item='';['move1','move2','move3','move4'].forEach(k=>next[i][k]='');emit(next)},DATA.map(x=>x.name),'一覧から選択')),
      ...
      h('button',{type:'button',onClick:add,style:{width:'100%',padding:'12px',border:'1px solid #1570ef',background:'#eff8ff',color:'#175cd3',fontWeight:'700',borderRadius:'8px'}},'＋ 育成論を追加')
    ])
  }
